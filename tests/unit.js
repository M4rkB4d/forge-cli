...
// ──────────────────────────────────────────────
{
  ...
  // Add protected files that should NOT be touched (generated by package managers)
  writeFileSync(join(cleanDir, 'package-lock.json'), '{"lockfileVersion":3}', 'utf-8');
  writeFileSync(join(cleanDir, '.env.local'), 'DB_PASSWORD=secret', 'utf-8');
  ...
  const PROTECTED_FILE_NAMES = new Set(['package-lock.json', 'yarn.lock', 'pnpm-lock.yaml', '.env', '.env.local']);
  ...
  const detectedOrphans = allProjectFiles.filter(f => {
    const fileName = f.split('/').pop();
    return !templatePaths.has(f) && !PROTECTED_FILE_NAMES.has(fileName);
  });
  ...
  // Test 5: After clean, no non-protected orphans remain
  const afterClean = projectWalk(cleanDir);
  const remainingOrphans = afterClean.filter(f => {
    const fn = f.split('/').pop();
    return !templatePaths.has(f) && !PROTECTED_FILE_NAMES.has(fn);
  });
  ...
  // Test 7: Protected files preserved
  assert(
    existsSync(join(cleanDir, 'package-lock.json')),
    suite,
    'package-lock.json preserved after clean',
  );
  assert(
    existsSync(join(cleanDir, '.env.local')),
    suite,
    '.env.local preserved after clean',
  );

  // Test 8: Template files still intact
  ...
  // Test 9: Running clean on already-clean project finds 0 orphans
  const cleanAgain = projectWalk(cleanDir).filter(f => {
    const fn = f.split('/').pop();
    return !templatePaths.has(f) && !PROTECTED_FILE_NAMES.has(fn);
  });
